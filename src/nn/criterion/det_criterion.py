...
class DetCriterion(torch.nn.Module):
    ...
    def _get_positive_nums(self, indices):
        ...
        num_pos = torch.clamp(
            num_pos / dist_utils.get_world_size(), min=1).item()
        return num_pos
    ...
    def loss_labels_focal(self, outputs, targets, indices, num_boxes):
        ...
        target = F.one_hot(
            target_classes, num_classes=self.num_classes + 1)[..., :-1].to(src_logits.dtype)
    ...
    def loss_labels_vfl(self, outputs, targets, indices, num_boxes):
        ...
        weight = self.alpha * \
            src_score.pow(self.gamma) * (1 - target) + target_score
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        target_boxes = torchvision.ops.box_convert(
            target_boxes, in_fmt=self.box_fmt, out_fmt='xyxy')
        loss_giou = 1 - \
            box_ops.elementwise_generalized_box_iou(src_boxes, target_boxes)
