...
class RTDETRCriterionv2(nn.Module):
    ...
    def loss_labels_vfl(self, outputs, targets, indices, num_boxes, values=None):
        ...
        weight = self.alpha * \
            pred_score.pow(self.gamma) * (1 - target) + target_score
    ...
    def forward(self, outputs, targets, **kwargs):
        ...
        outputs_without_aux = {k: v for k,
                               v in outputs.items() if 'aux' not in k}
        ...
        for loss in self.losses:
            ...
            l_dict = self.get_loss(
                loss, outputs, targets, indices, num_boxes, **meta)
            ...
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                for loss in self.losses:
                    meta = self.get_loss_meta_info(
                        loss, aux_outputs, targets, indices)
                    l_dict = self.get_loss(
    ...
    def get_loss_meta_info(self, loss, outputs, targets, indices):
        ...
        src_boxes = outputs['pred_boxes'][self._get_src_permutation_idx(
            indices)]
        target_boxes = torch.cat([t['boxes'][j]
                                 for t, (_, j) in zip(targets, indices)], dim=0)

        if self.boxes_weight_format == 'iou':
            iou, _ = box_iou(box_cxcywh_to_xyxy(
                src_boxes.detach()), box_cxcywh_to_xyxy(target_boxes))
            iou = torch.diag(iou)
