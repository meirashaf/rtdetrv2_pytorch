...
class MSDeformableAttention(nn.Module):
    def __init__(
        ...
        self.register_buffer('num_points_scale', torch.tensor(
            num_points_scale, dtype=torch.float32))
    ...
    def _reset_parameters(self):
        ...
        scaling = torch.concat([torch.arange(1, n + 1)
    ...
    def forward(self,
        ...
        attention_weights = F.softmax(
            attention_weights, dim=-1).reshape(bs, Len_q, self.num_heads, sum(self.num_points_list))

        if reference_points.shape[-1] == 2:
            offset_normalizer = torch.tensor(value_spatial_shapes)
            offset_normalizer = offset_normalizer.flip(
                [1]).reshape(1, 1, 1, self.num_levels, 1, 2)
    ...
    def forward(self,
        ...
        for i, layer in enumerate(self.layers):
            ...
            output = layer(output, ref_points_input, memory,
                           memory_spatial_shapes, attn_mask, memory_mask, query_pos_embed)
...
class RTDETRTransformerv2(nn.Module):
    ...
    def _generate_anchors(self,
        ...
        for lvl, (h, w) in enumerate(spatial_shapes):
            ...
            lvl_anchors = torch.concat(
                [grid_xy, wh], dim=-1).reshape(-1, h * w, 4)
    ...
    def _get_decoder_input(self,
        ...
        enc_outputs_coord_unact: torch.Tensor = self.enc_bbox_head(
            output_memory) + anchors

        enc_topk_bboxes_list, enc_topk_logits_list = [], []
        enc_topk_memory, enc_topk_logits, enc_topk_bbox_unact = \
            self._select_topk(output_memory, enc_outputs_logits,
                              enc_outputs_coord_unact, self.num_queries)
        ...
        if self.learn_query_content:
            content = self.tgt_embed.weight.unsqueeze(
                0).tile([memory.shape[0], 1, 1])
        else:
